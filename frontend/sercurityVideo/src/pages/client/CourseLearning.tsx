...
export default function Course() {
    ...
    useEffect(() => {
        if (id) {
            fetchDetail();
            window.scrollTo(0, 0); // Reset scroll khi vào trang mới
        }
    }, [id]);

    // Reset scroll khi đổi bài học để tránh tình trạng nhảy trang, dùng 'auto' để lập tức quay về đầu
    useEffect(() => {
    ...
    return (
        <div style={{ background: '#f8fafc', minHeight: '100vh', margin: '-24px -40px' }}>
            <Row gutter={0}>
                <Col lg={16} md={24} style={{ padding: '24px', minHeight: '100vh', background: '#f8fafc' }}>
                    <div style={{ maxWidth: '1000px', margin: '0 auto' }}>
                        <Button
                            icon={<LeftOutlined />}
                            onClick={() => navigate(`/course/${id}`)}
                            style={{ marginBottom: 20, background: 'transparent', color: 'black', border: 'none' }}
                        >
                ...
                <Col lg={8} md={24} style={{ background: '#fff', borderLeft: '1px solid #e2e8f0', minHeight: '100vh' }}>
                    <div style={{ padding: '24px', borderBottom: '1px solid #e2e8f0', position: 'sticky', top: '66px', background: '#fff', zIndex: 5 }}>
